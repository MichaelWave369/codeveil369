...
function App() {
  ...
  const [status, setStatus] = useState("CodeVeil369 v0.6 snapshot hash ready.");
  ...
  const shareText = useMemo(() => [`CodeVeil369 snapshot: ${currentHash}`, `Preset: ${activePreset?.name ?? "custom / loaded JSON"}`, `Mode: ${params.mode} · Material focus: ${params.material} · Seed: ${params.seed}`, `Ledger tier: ${receipts[tier].label}`, `Boundary: ${receipts[tier].boundary}`, "This is a symbolic perception simulation, not proof of external code."].join("\n"), [activePreset, currentHash, params.mode, params.material, params.seed, tier]);
  ...
  function writeGallery(next: PresetLedgerEntry[]) {
    setGallery(next);
    try { localStorage.setItem(GALLERY_KEY, JSON.stringify(next)); } catch { setStatus("Preset gallery could not be saved in this browser."); }
  }
