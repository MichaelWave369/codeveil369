...
export function downloadPresetLedger(entry: PresetLedgerEntry) {
  ...
  link.href = url;
  link.download = `codeveil369-preset-${entry.presetId}-${entry.params.seed}.json`;
  link.click();
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (parsed?.schema !== "codeveil369.preset.v1" || !parsed?.params) throw new Error("Invalid CodeVeil369 preset ledger file.");
